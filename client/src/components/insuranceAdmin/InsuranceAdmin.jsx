...
const InsuranceAdmin = (props) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3001/${insCompany}`).then((res) => {
      if (res.status === 200) {
        setClients(res.data);
      }
    });
  ...
  const findBy = (criteria) => {
    switch (criteria) {
      case "date":
        axios
          .post("http://localhost:3001/query/date", {
      ...
      case "name":
        axios
          .post("http://localhost:3001/query/name", {
            insuranceCompany: insCompany,
            name: queryData.name,
          })
          .then((res) => {
        ...
        break;
      case "idn":
        console.log(queryData.idn);
        axios
          .post("http://localhost:3001/query/idn", {
            insuranceCompany: insCompany,
            idn: queryData.idn,
          })
          .then((res) => {
      ...
      case "accident":
        ...
        axios
          .post("http://localhost:3001/query/accidentType", {
            insuranceCompany: insCompany,
  ...
  return insCompany === "A" || insCompany === "B" || insCompany === "C" ? (
    <section className="admin-section">
      <div className="admin-panel">
        ...
        <div className="input-buttons">
          <button className="btn btn-secondary" onClick={() => findBy("date")}>
            {translate("FIND")}
          </button>
          <button className="btn btn-secondary" onClick={() => findBy("name")}>
            {translate("FIND")}
          </button>
          <button className="btn btn-secondary" onClick={() => findBy("idn")}>
            {translate("FIND")}
          </button>
          <button
            className="btn btn-secondary"
            onClick={() => findBy("accident")}
          >
            {translate("FIND")}
          </button>
        </div>
      ...
      {clients.map((item) => (
        <div>
          ...
          <button
            ...
            onClick={() => payClient(item)}
          >
            {translate("Pay client")}
          </button>
        </div>
      ))}
    </section>
